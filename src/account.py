...
class Account(object):
    ...
    def __repr__(self):
        return "Account %d (%s %s) balance %d" % (
            self.id,
            self.host,
            self.sub,
            self._balance,
    ...
    def __eq__(self, other):
        ...
        if (
            ...
            or self.banker != other.banker
            or self.oracle != other.oracle
        ):
            return False
    ...
    def persist(self, db=None):
        ...
        with db.conn.cursor() as curs:
            if self.id is not None:
                ...
                curs.execute(
                    """INSERT INTO account (system, banker, oracle, enabled, balance)
            ...
            if self.host is not None or self.sub is not None:
                ...
                if self.username is None or self.profile is None:
                    ...
                    return None
                curs.execute(
                    """INSERT INTO userid (account, host, sub, username, profile)
    ...
    def total(self):
        ...
        with self.db.conn.cursor() as curs:
            ...
            curs.execute(
                "SELECT price, quantity FROM offer WHERE account = %s AND side = %s",
    ...
    def get_by_oauth(cls, curs, host, sub, db=None):
        curs.execute(
            """SELECT account.id, enabled, balance, username, profile, banker, oracle
                        FROM account JOIN userid ON account.id = userid.account
                        WHERE userid.host = %s AND userid.sub = %s""",
            (host, sub),
        )
        if curs.rowcount == 1:
            (uid, enabled, balance, username, profile, banker, oracle) = curs.fetchone()
            return cls(
                uid=uid,
                banker=banker,
                oracle=oracle,
