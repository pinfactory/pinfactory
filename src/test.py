...
try:
    ...
    print(
        "This should be run on the server or container with Python dependencies installed."
    )
...
class MarketTestCase(unittest.TestCase):
    ...
    def make_contract_type(testdb, when=None):
        ...
        testissue = Issue(url="http://bug.example.com/%s/" % uuid.uuid4()).persist(
            testdb
        )
    ...
    def test_db_time(self):
        ...
        self.assertAlmostEqual(
            testdb.now().timestamp(), datetime.now().timestamp(), delta=0.2
        )
    ...
    def test_persist_and_lookup_account(self):
        ...
        testdb = Market()
        testuser = Account(
            host="GitHub", sub=31337, username="jrdobbs", profile="https://bob.dobbs/"
        )
    ...
    def test_add_upcoming_maturities(self):
        ...
        for i in range(len(result) - 1):
            self.assertEqual(
                result[i].maturity + timedelta(days=14), result[i + 1].maturity
            )
    ...
    def test_add_offer(self):
        ...
        good_test_offer = testdb.offer(
            testuser, test_contract_type, Market.FIXED, 500, 100
        )
        msglist = good_test_offer.place()
        result = testdb.offer.filter(issue=test_contract_type.issue)
        self.assertEqual(
            good_test_offer.contract_type.maturity.maturity,
    ...
    def test_lookup_offer_by_id_and_account(self):
        ...
        both_offer = market.offer.filter(
            issue=test_contract_type.issue, account=testuser
        )[0]
    ...
    def test_add_and_cancel_offer(self):
        ...
        test_contract_type = self.make_contract_type(testdb)
        mlist = testdb.offer(
            testuser, test_contract_type, Market.FIXED, 100, 10
        ).place()
    ...
    def test_fail_add_late_offer(self):
        ...
        testdb = Market()
        testuser = Account(balance=10000).persist(testdb)
        test_contract_type = self.make_contract_type(
            testdb, when=testdb.now() - timedelta(seconds=1)
        )
    ...
    def test_add_matching_offers_user_first(self):
        ...
        self.assertEqual(1, len(mlist))
        self.assertEqual(
            ("offer_created", testuser.id, Market.UNFIXED, 100, 10),
            (
                mlist[0].mclass,
                mlist[0].account,
                mlist[0].side,
        ...
        self.assertEqual(
            ("contract_created", testfixer.id, Market.FIXED, 100, 10),
            (
                mlist[0].mclass,
                mlist[0].account,
                mlist[0].side,
        ...
        self.assertEqual(
            ("contract_created", testuser.id, Market.UNFIXED, 100, 10),
            (
                mlist[1].mclass,
                mlist[1].account,
                mlist[1].side,
    ...
    def test_add_matching_offers_fixer_first(self):
        ...
        self.assertEqual(1, len(mlist))
        self.assertEqual(
            ("offer_created", testfixer.id, Market.FIXED, 100, 20),
            (
                mlist[0].mclass,
                mlist[0].account,
                mlist[0].side,
        ...
        self.assertEqual(2, len(mlist))
        self.assertEqual(
            ("contract_created", testfixer.id, Market.FIXED, 100, 20),
            (
                mlist[0].mclass,
                mlist[0].account,
                mlist[0].side,
        ...
        self.assertEqual(
            ("contract_created", testuser.id, Market.UNFIXED, 100, 20),
            (
                mlist[1].mclass,
                mlist[1].account,
                mlist[1].side,
    ...
    def test_add_part_match_existing(self):
        ...
        self.assertEqual(1, len(mlist))
        self.assertEqual(
            ("offer_created", testuser.id, Market.UNFIXED, 100, 80),
            (
                mlist[0].mclass,
                mlist[0].account,
                mlist[0].side,
        ...
        self.assertEqual(2, len(mlist))
        self.assertEqual(
            ("contract_created", testfixer.id, Market.FIXED, 100, 50),
            (
                mlist[0].mclass,
                mlist[0].account,
                mlist[0].side,
        ...
        self.assertEqual(
            ("contract_created", testuser.id, Market.UNFIXED, 100, 50),
            (
                mlist[1].mclass,
                mlist[1].account,
                mlist[1].side,
    ...
    def test_add_part_match_new(self):
        ...
        self.assertEqual(
            ("contract_created", testfixer.id, Market.FIXED, 250, 10),
            (
                mlist[0].mclass,
                mlist[0].account,
                mlist[0].side,
        ...
        self.assertEqual(
            ("contract_created", testuser.id, Market.UNFIXED, 250, 10),
            (
                mlist[1].mclass,
                mlist[1].account,
                mlist[1].side,
        ...
        self.assertEqual(
            ("offer_created", testfixer.id, Market.FIXED, 250, 15),
            (
                mlist[2].mclass,
                mlist[2].account,
                mlist[2].side,
    ...
    def test_no_deal_user_first(self):
        ...
        testdb.offer(testuser, test_contract_type, Market.UNFIXED, 200, 10).place()
        mlist = testdb.offer(
            testfixer, test_contract_type, Market.FIXED, 100, 10
        ).place()
    ...
    def test_no_deal_fixer_first(self):
        ...
        testdb.offer(testfixer, test_contract_type, Market.FIXED, 100, 10).place()
        mlist = testdb.offer(
            testuser, test_contract_type, Market.UNFIXED, 200, 10
        ).place()
        self.assertEqual("offer_created", mlist[0].mclass)
        self.assertEqual(8000, testuser.total)
        self.assertEqual(1000, testfixer.total)
    ...
    def test_add_match_many(self):
        ...
        self.assertEqual(
            ("contract_created", testuser1.id, Market.UNFIXED, 100, 10),
            (
                mlist[1].mclass,
                mlist[1].account,
                mlist[1].side,
        ...
        self.assertEqual(
            ("contract_created", testuser2.id, Market.UNFIXED, 100, 10),
            (
                mlist[3].mclass,
                mlist[3].account,
                mlist[3].side,
    ...
    def test_add_match_best_user_offers(self):
        ...
        good_offer_1 = testdb.offer(
            testuser1, test_contract_type, Market.UNFIXED, 100, 10
        )
        good_offer_1.place()
        good_offer_2 = testdb.offer(
            testuser2, test_contract_type, Market.UNFIXED, 100, 10
        )
        good_offer_2.place()
    ...
    def test_offset_offers(self):
        ...
        mlist = testdb.offer(
            testfixer, test_contract_type, Market.UNFIXED, 950, 10
        ).place()
        self.assertEqual(4, len(mlist))
    ...
    def test_crowdfunding(self):
        ...
        test_contract_type = self.make_contract_type(testdb)
        testdb.offer(
            testfixer, test_contract_type, Market.FIXED, 500, user_count
        ).place()
        ...
        for i in range(user_count):
            testdb.offer(
                testusers[i], test_contract_type, Market.UNFIXED, 500, 1
            ).place()
        ...
        for position in positions:
            ...
            self.assertEqual(
                timezone.utc.utcoffset(m),
                position.contract_type.maturity.tzinfo.utcoffset(m),
            )
    ...
    def test_partial_work(self):
        ...
        testdb.offer(
            css_developer, test_contract_type, Market.UNFIXED, 950, 100
        ).place()
    ...
    def test_fail_partly_match_existing_all_or_nothing_offer(self):
        ...
        testoffer = testdb.offer(
            testfixer, test_contract_type, Market.FIXED, 500, 100, all_or_nothing=True
        )
        testoffer.place()
        self.assertEqual(0, testfixer.balance)
        test_user_offer = testdb.offer(
            testuser, test_contract_type, Market.UNFIXED, 500, 25
        )
