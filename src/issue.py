...
class Issue(object):
    def __init__(self, iid=None, modified=None, url=None, title=None, is_open=True, offer_volume=0):
        self.db = None
        if iid:
            self.id = int(iid)
    ...
    def persist(self, db):
        ...
        with db.conn.cursor() as curs:
            curs.execute('''INSERT INTO issue (url, title, open) VALUES (%s, %s, %s)
            ...
            if self.title is None:
                ...
                curs.execute('''UPDATE issue SET title = %s WHERE id = %s''',
                    (self.title, self.id))
            if inserted:
    ...
    def by_id(cls, db, iid):
        with db.conn.cursor() as curs:
            curs.execute("SELECT url, title, modified, open FROM issue WHERE id = %s", (iid,))
            if not curs.rowcount:
                return None
            (url, title, modified, is_open) = curs.fetchone()
            return cls(iid=iid, modified=modified, url=url, title=title, is_open=is_open)

    @classmethod
    def get_all(cls, db, include_private=False):
        with db.conn.cursor() as curs:
            curs.execute('''SELECT issue.id, issue.modified, issue.url, issue.title, issue.open,
                            SUM(offer.quantity)
            ...
            for row in curs.fetchall():
                ...
                tmp = cls(iid=iid, modified=modified, url=url, title=title,
                          is_open=is_open, offer_volume=volume)
